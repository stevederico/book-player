...
const t0 = Date.now();
let lastTick = 0;
const { audioMp3, words, totalDuration, transcript } = await synthesizeGuide({
...
const outPath = resolve(outDir, `${slug}.mp3`);
...
db.prepare('UPDATE Guides SET timing_json = ?, duration = ?, audio_url = ?, transcript = ? WHERE slug = ?')
  .run(timingJson, Math.round(totalDuration), `/audio/${slug}.mp3`, transcript, slug);

console.log(`[regen] done in ${((Date.now() - t0) / 1000).toFixed(1)}s — duration=${totalDuration.toFixed(1)}s words=${words.length} mp3=${outPath}`);
