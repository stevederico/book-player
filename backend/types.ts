...
// ==== GUIDE (BOOK-PLAYER) SHAPES ====
//
// App-specific domain for book-player: an audio "guide" with chapters,
...
/**
 * Async job state for a guide, keyed by step name (e.g. 'tts',
 * 'chapter-images'). Each step holds an open-ended status record
 * (e.g. `{ status, startedAt, error }`), shallow-merged by updateGuideJob.
 */
export type GuideJobs = Record<string, Record<string, unknown>>;

/** A single word with its start time (seconds) in the synthesized audio. */
export interface GuideWordTiming {
  /** Word text. */
  w: string;
...
/**
 * A chapter entry. Core fields (title/quote/caption/time) are produced by the
 * analyze + chapter-timing pipeline; image/realImage are attached by the
 * image steps. Open-ended via an index signature because the producing
 * pipelines may add more fields over time.
 */
export interface GuideChapter {
  /** Start time in seconds (0 for the first chapter). */
  time?: number;
  /** Chapter title. */
  title?: string;
  /** Source quote used to locate the chapter. */
  quote?: string;
  /** Optional caption. */
  caption?: string;
  /** Generated chapter image metadata; `generated` holds the image path once produced. */
  image?: { generated?: string | boolean } & Record<string, unknown>;
  /** Real (e.g. Unsplash) chapter image metadata. */
  realImage?: unknown;
...
/**
 * Word-timing payload stored on a guide. Normally `{ words: [...] }`; kept
 * open-ended (index signature) because legacy/imported payloads vary.
 */
export interface GuideTiming {
  /** Per-word start times. */
  words?: GuideWordTiming[];
  /** Additional payload fields. */
  [key: string]: unknown;
}

/**
 * Optional filters for listGuides. `visibility` narrows to one visibility
 * bucket (e.g. 'public', 'unlisted').
 */
export interface GuideFilters {
  /** Restrict to guides with this visibility value. */
  visibility?: string;
}

/** Options controlling which heavy fields shapeGuide includes (all default true). */
export interface ShapeGuideOptions {
  /** Include the full transcript text. */
  includeTranscript?: boolean;
  /** Include the word-timing payload. */
  includeTiming?: boolean;
  /** Include the chapters array body. */
  includeChapters?: boolean;
}

/**
 * Upsert payload accepted by upsertGuide / POST /api/guides. `slug` and
 * `title` are required; everything else is optional and defaulted by the
 * adapter.
 */
export interface GuideInput {
  ...
  /** Audio URL (stored as audio_url). */
  audio?: string | null;
  /** Thumbnail URL. */
  thumbnail?: string | null;
  /** Word-timing offset in seconds. */
  timingOffset?: number | null;
  /** Player default view mode (e.g. 'real'). */
  ...
  /** Word-timing payload. */
  timing?: GuideTiming | null;
  /** Visibility bucket (defaults to 'public'). */
  visibility?: string;
  /** Owning user `_id`. */
  createdBy?: string | null;
}

/** Summary row returned by listGuides (no transcript/timing/chapter body). */
...
/** Fully hydrated guide returned by getGuide / shapeGuide. */
export interface Guide {
  ...
  createdAt: number;
  updatedAt: number;
  /** Present unless excluded via ShapeGuideOptions.includeChapters. */
  chapters?: GuideChapter[];
  /** Present unless excluded via ShapeGuideOptions.includeTranscript. */
  transcript?: string;
  /** Present only when timing data exists and includeTiming is not false. */
  timing?: GuideTiming;
}

/** Result of upsertGuide: the slug and whether a new row was inserted. */
...
export interface DatabaseProvider<TDb = unknown> {
  ...
  // --- Guide methods (book-player) — implemented by SQLiteProvider only. ---
  ...
  getGuide?(db: TDb, slug: string): Promise<Guide | null>;
  /** Insert or update a guide by slug. */
  upsertGuide?(db: TDb, guide: GuideInput): Promise<UpsertGuideResult>;
  /** Atomically merge a job state into a guide's jobs payload. */
  updateGuideJob?(db: TDb, slug: string, step: string, jobState: Record<string, unknown>): Promise<GuideJobs>;
}
...
export interface BoundDatabase {
  ...
  // --- Guide methods (book-player), pre-bound with dbType/db/connectionString. ---
  ...
  /** Insert or update a guide by slug. */
  upsertGuide(guide: GuideInput): Promise<UpsertGuideResult>;
  /** Atomically merge a job state into a guide's jobs payload. */
  updateGuideJob(slug: string, step: string, jobState: Record<string, unknown>): Promise<GuideJobs>;
}
