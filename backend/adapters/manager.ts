 ...
 */
type GuideCapableProvider = Required<Pick<DatabaseProvider,
  'listGuides' | 'getGuide' | 'upsertGuide' | 'updateGuideJob'>>;

/**
 * Assert that `provider` implements the guide methods, returning it narrowed.
 *
 * @throws {Error} If the configured provider does not support guides.
 */
function asGuideProvider(provider: DatabaseProvider): GuideCapableProvider {
  if (!provider.listGuides || !provider.getGuide || !provider.upsertGuide || !provider.updateGuideJob) {
    throw new Error('Configured database provider does not support guide operations');
  }
  return provider as GuideCapableProvider;
}
...
class DatabaseManager {
   ...
   */
  async listGuides(dbType: string, dbName: string, connectionString: string, filters: GuideFilters = {}): Promise<GuideSummary[]> {
    const { provider, database } = await this.getDatabase(dbType, dbName, connectionString);
    return await asGuideProvider(provider).listGuides(database, filters);
  }
   ...
   */
  async getGuide(dbType: string, dbName: string, connectionString: string, slug: string): Promise<Guide | null> {
    const { provider, database } = await this.getDatabase(dbType, dbName, connectionString);
    return await asGuideProvider(provider).getGuide(database, slug);
  }
   ...
   */
  async upsertGuide(dbType: string, dbName: string, connectionString: string, guide: GuideInput): Promise<UpsertGuideResult> {
    const { provider, database } = await this.getDatabase(dbType, dbName, connectionString);
    return await asGuideProvider(provider).upsertGuide(database, guide);
  }
   ...
   */
  async updateGuideJob(dbType: string, dbName: string, connectionString: string, slug: string, step: string, jobState: Record<string, unknown>): Promise<GuideJobs> {
    const { provider, database } = await this.getDatabase(dbType, dbName, connectionString);
    return await asGuideProvider(provider).updateGuideJob(database, slug, step, jobState);
  }
